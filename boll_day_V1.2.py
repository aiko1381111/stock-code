...
def calculate_bollinger_bands_with_volume_and_ma(data, window=20, num_std=2, volume_multiplier=1.3):
    """
    计算布林带指标并分析成交量，增加120日均线
    """
    df = data.copy()
    ...
    df['BOLL_Upper'] = df['MA20'] + num_std * df['STD']  # 布林上轨
    df['BOLL_Lower'] = df['MA20'] - num_std * df['STD']  # 布林下轨
    ...
    # 判断20日线方向（今日MA20 > 昨日MA20 表示向上）
    df['ma20_direction'] = df['MA20'] > df['MA20'].shift(1)

    # 新增：判断是否低于120日均线
    df['below_ma120'] = df['low'] < df['MA120']
...
def find_buy_signals_optimized(data, volume_multiplier=1.2):
    ...
    修改后的买入条件：
    1. 最低价 ≤ 布林下轨 × 0.995 (低于下轨0.5%)
    2. 最低价 < 120日均线
    3. 成交量显著放大（当日成交量 > 5日均量 × volume_multiplier）- 可选条件
    """
    buy_signals = []
    ...
    for i in range(1, len(data)):
        ...
        # 检查是否有有效的布林带数据和MA120数据
        if pd.isna(current_row['BOLL_Lower']) or pd.isna(current_row['MA120']):
            continue
        ...
        # 检查买入条件
        condition1 = current_row['low'] <= boll_lower_threshold  # 最低价低于布林下轨0.5%
        condition2 = current_row['below_ma120']  # 最低价低于120日均线
        condition4 = current_row['pctChg'] <= -1  # 单日跌幅 ≥ 1%


        # 成交量显著放大（当日成交量大于5日均量的volume_multiplier倍）- 可选
        condition3 = current_row['volume_ratio'] >= volume_multiplier if not pd.isna(
            current_row['volume_ratio']) else False

        # 核心条件：必须满足条件1和条件2，条件3可选
        if condition1 and condition2 and condition4:
            ...
            # 计算低于120日均线的百分比
            below_ma120_pct = ((current_row['close'] - current_row['MA120']) / current_row['MA120'] * 100)

            # 计算120日均线相对于20日均线的位置
            ma120_vs_ma20_pct = ((current_row['MA120'] - current_row['MA20']) / current_row['MA20'] * 100)
            ...
            buy_signal = {
                ...
                'ma120_price': round(current_row['MA120'], 2),  # 120日均线价格
                'below_ma120_pct': round(below_ma120_pct, 2),  # 低于120日均线的百分比
                'ma120_vs_ma20_pct': round(ma120_vs_ma20_pct, 2),  # MA120相对于MA20的位置
                ...
                'current_ma20': round(current_row['MA20'], 2),
                'boll_upper': round(current_row['BOLL_Upper'], 2) if 'BOLL_Upper' in current_row else None,
                'ma20_direction': '向上' if current_row['ma20_direction'] else '向下',
                'below_ma120': '是' if current_row['below_ma120'] else '否',
                'volume_condition_met': '是' if condition3 else '否'  # 记录成交量条件是否满足
            }
...
def save_results_to_excel(all_results, filename=None):
    ...
    if filename is None:
        ...
        filename = os.path.join(desktop, f"股票买入信号分析_布林带策略_{timestamp}.xlsx")
    ...
    preferred_columns = ['stock_code', 'stock_name', 'date', 'sell_date', 'cost_price', 'sell_price',
                         ...
                         'below_ma120_pct', 'ma120_vs_ma20_pct', 'below_ma120', 'volume_condition_met',
    ...
    column_mapping = {
        ...
        'close_price': '买入收盘价',
        'boll_lower': '布林下轨',
        'below_boll_pct': '低于布林下轨%',
        'ma120_price': '120日均线',
        'below_ma120_pct': '低于120日均线%',
        'ma120_vs_ma20_pct': 'MA120相对于MA20%',
        'below_ma120': '是否低于MA120',
        'volume_condition_met': '成交量条件满足',
        'current_ma20': '20日均线',
        'boll_upper': '布林上轨',
        'pctChg': '买入跌幅%',
        'holding_days': '持仓天数',
        ...
        with pd.ExcelWriter(filename, engine='openpyxl') as writer:
            ...
            for result in all_results:
                if result is not None and result['buy_signals']:
                    ...
                    # 按成交量条件分类统计
                    volume_condition_met = len(
                        [s for s in result['buy_signals'] if s.get('volume_condition_met') == '是'])
                    volume_condition_not_met = len(
                        [s for s in result['buy_signals'] if s.get('volume_condition_met') == '否'])
                    ...
                    # 计算平均低于120日均线的百分比
                    below_ma120_list = [s.get('below_ma120_pct', 0) for s in result['buy_signals']]
                    avg_below_ma120 = np.mean(below_ma120_list) if below_ma120_list else 0

                    summary_data.append({
                        '股票代码': result['stock_code'],
                        '股票名称': result['stock_name'],
                        '买入信号数量': total_signals,
                        '成交量达标信号': volume_condition_met,
                        '成交量未达标信号': volume_condition_not_met,
                        '盈利信号数量': profitable_signals,
                        '成功率%': round(success_rate, 2),
                        '平均持仓天数': round(avg_holding_days, 1),
                        '平均收益率%': round(avg_profit_pct, 2),
                        '平均低于布林下轨%': round(avg_below_boll, 2),
                        '平均低于120日均线%': round(avg_below_ma120, 2)
                    })
...
def analyze_stock_buy_signals_optimized(stock_code, stock_name, years=5, volume_multiplier=1.3):
    """
    ...
    # 计算布林带指标和120日均线（优化版）
    stock_data_with_boll = calculate_bollinger_bands_with_volume_and_ma(stock_data, volume_multiplier=volume_multiplier)
    ...
    if buy_signals_with_holding:
        ...
        print("-" * 240)
        print(
            f"{'日期':<12} {'成本价':<8} {'布林下轨':<9} {'低于下轨%':<10} {'120日均线':<10} "
            f"{'低于MA120%':<12} {'成交量比':<8} {'持仓天数':<8} {'卖出价':<8} {'收益%':<10} {'状态':<10} {'卖出原因':<25}")
        print("-" * 240)
        ...
        for signal in buy_signals_with_holding:
            ...
            ma120_price = str(signal.get('ma120_price', '0'))
            below_ma120_pct = f"{signal.get('below_ma120_pct', 0):.2f}%"
            ...
            line = (f"{date_str:<12} {cost_price:<8} {boll_lower:<9} {below_boll_pct:<10} "
                    f"{ma120_price:<10} {below_ma120_pct:<12} {volume_ratio:<8} {holding_days:<8} "
                    f"{sell_price_display:<8} {profit_str:<10} {status:<10} {sell_reason:<25}")
            print(line)
        ...
        # 按成交量条件分类统计
        volume_met_signals = len([s for s in buy_signals_with_holding if s.get('volume_condition_met') == '是'])
        volume_not_met_signals = len([s for s in buy_signals_with_holding if s.get('volume_condition_met') == '否'])
        ...
        avg_below_ma120 = np.mean(
            [s.get('below_ma120_pct', 0) for s in buy_signals_with_holding]) if total_signals > 0 else 0

        print(f"\n📊 统计摘要:")
        print(f"   总信号数: {total_signals}")
        print(f"   成交量达标信号: {volume_met_signals}, 成交量未达标信号: {volume_not_met_signals}")
        ...
        print(f"   平均低于120日均线: {avg_below_ma120:.2f}%")
        print(
...
if __name__ == "__main__":
    # 定义要分析的股票列表
    stock_configs = [
        {"code": "sh.600031", "name": "三一重工"},   #买入价，不得超过24.74
        #{"code": "sh.601088", "name": "中国神华"},  #买入价，不得超过35.54
        {"code": "sh.600585", "name": "海螺水泥"},  #买入价，不得超过21.99
        #{"code": "sh.601985", "name": "中国核电"},  #买入价，不得超过5.8
        #{"code": "sh.603288", "name": "海天味业"},  #买入价，不得超过35.27
        #{"code": "sz.000425", "name": "徐工集团"},  #买入价，不得超过7.92
        #{"code": "sh.601939", "name": "建设银行"},
        #{"code": "sh.601398", "name": "工商银行"},
         #{"code": "sh.601288", "name": "农业银行"},
         #{"code": "sh.601988", "name": "中国银行"},
         #{"code": "sh.600941", "name": "中国移动"},
         {"code": "sh.600900", "name": "长江电力"}   #买入价，不得超过19.07
    ]

    print(f"{'🎯 开始分析股票买入信号 (优化策略) ':~^120}")
    print("买入条件：")
    print("1. 收盘价 ≤ 布林下轨 × 0.995 (低于下轨0.5%)")
    print("2. 收盘价 < 120日均线")
    print("3. 成交量显著放大 (当日成交量 > 5日均量 × 1.2) - 可选条件")
    print("\n卖出条件：")
    print("1. 止盈：实时价 ≥ 布林上轨 × 1.02 (高于上轨2%)")
    print("2. 止损：实时价 ≤ 买入价 × 0.92 (低于买入价8%)")
    print("3. 最大持仓：180天强制平仓")
    print(f"{'~' * 120}")
    ...
    excel_path = os.path.join(desktop_path, "股票买入信号分析_布林带策略_含120日均线.xlsx")
